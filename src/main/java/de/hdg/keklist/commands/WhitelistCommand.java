...
public class WhitelistCommand implements BrigadierCommand {
    ...
    public @NotNull LiteralCommandNode<CommandSourceStack> getCommand() {
                ...
                .then(Commands.argument("entry", StringArgumentType.word())
                        .suggests((ctx, builder) -> {
                            Bukkit.getOnlinePlayers().forEach(player -> {
                                try (ResultSet rsUser = Keklist.getDatabase().onQuery("SELECT * FROM whitelist WHERE name = ?", player.getName());
                                     ResultSet rsIp = Keklist.getDatabase().onQuery("SELECT * FROM whitelistIp WHERE ip = ?", player.getAddress().getAddress().getHostAddress())
                                ) {
                                    if (!rsUser.next())
        ...
        LiteralArgumentBuilder<CommandSourceStack> removeSubcommand = Commands.literal("remove")
                .then(Commands.argument("entry", StringArgumentType.word())
                        .suggests((ctx, builder) -> {
                            try (ResultSet rs = Keklist.getDatabase().onQuery("SELECT * FROM (SELECT name AS entry FROM whitelist UNION ALL SELECT ip AS entry FROM whitelistIp UNION ALL SELECT domain AS entry FROM whitelistDomain) as whitelistEntries");
                            ) {
                                List<String> whitelistEntries = new ArrayList<>();

                                while (rs.next()) {
                                    whitelistEntries.add(rs.getString(1));
                                }
        ...
        LiteralArgumentBuilder<CommandSourceStack> levelSubcommand = Commands.literal("level")
                .then(Commands.argument("entry", StringArgumentType.word())
                        .suggests((ctx, builder) -> {
                            ...
                            try (ResultSet entriesRs = Keklist.getDatabase().onQuery("SELECT entry FROM whitelistLevel")) {
                                while (entriesRs.next()) {
                                    levelEntries.add(entriesRs.getString(1));
                                }
                            } catch (SQLException ex) {
                                ex.printStackTrace();
                            }
            ...
            switch (ctx.getNodes().getFirst().getNode().getName()) {
                case "add" -> {
                    ...
                    switch (type) {
                        ...
                        case IPv4, IPv6 -> {
                            ResultSet rs = Keklist.getDatabase().onQuery("SELECT * FROM whitelistIp WHERE ip = ?", entry);
                        ...
                        case DOMAIN -> {
                            ResultSet rs = Keklist.getDatabase().onQuery("SELECT * FROM whitelistDomain WHERE domain = ?", entry);
                ...
                case "remove" -> {
                    ...
                    switch (type) {
                        case JAVA, BEDROCK -> {
                            ResultSet rs = Keklist.getDatabase().onQuery("SELECT * FROM whitelist WHERE name = ?", entry);
                            if (rs.next()) {
                                ...
                                ResultSet rsUserFix = Keklist.getDatabase().onQuery("SELECT * FROM whitelist WHERE name = ?", entry + " (Old Name)");
                                if (rsUserFix.next()) {

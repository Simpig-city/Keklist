...
build=$(curl -s -L "https://api.purpurmc.org/v2/purpur/1.19.4/latest/" | jq -r '.build')
currentVersion=$(jq -r '.currentVersion' version_history.json)

echo "Latest build: git-Purpur-$build (MC: 1.19.4)"
echo "Current version: $currentVersion"
# Compare the two values and output the result
if [[ "git-Purpur-$build (MC: 1.19.4)" == "$currentVersion" ]]; then
  echo "The server build is up-to-date."
else
  echo "The build is out-of-date. Downloading latest one..."
  curl -L https://api.purpurmc.org/v2/purpur/1.19.4/latest/download -o server.jar
fi
